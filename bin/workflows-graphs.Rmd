...
workflows_url <- "https://raw.githubusercontent.com/galaxyproject/galaxy_codex/refs/heads/main/communities/microgalaxy/resources/workflows.tsv"
...
hc_y <- mm |> dist() |> hclust(method = "ward.D2")
hc_x <- mm |> t() |> dist() |> hclust(method = "ward.D2")
...
gr1 <- dt1 |>
    ...
    theme(
        axis.text.x = element_text(size = 1, angle = 90, hjust = 1, vjust = .5),
...
dt1$Source   <- ifelse(dt1$Source == "WorkflowHub", "WorkflowHub", "Public servers")
dt1$Projects <- ifelse(dt1$Projects == "Intergalactic Workflow Commission (IWC)", "IWC", "Other Projects") |> str_wrap(22)

dt1$cluster <- paste0(dt1$Source, "; ", dt1$Projects)
dt1$cluster <- ifelse(dt1$cluster == "WorkflowHub; IWC", "IWC on WorkflowHub",
                      ifelse(dt1$cluster == "WorkflowHub; Other Projects", "Other WorkflowHub workflow", "Public server workflows")) |> str_wrap(15)
...
dt1$`EDAM operations` <- dt1$`EDAM operations` |> str_wrap(15)
...
ggsave(
    plot = gr2, filename = paste0(outfolder, "/heatmap-groups.png"),
    width = 12, height = 10, units = "in", dpi = 600
)

ggsave(
    plot = gr2, filename = paste0(outfolder, "/heatmap-groups.svg"),
    width = 12, height = 10, units = "in"
)

ggsave(
    plot = gr2, filename = paste0(outfolder, "/heatmap-groups.pdf"),
